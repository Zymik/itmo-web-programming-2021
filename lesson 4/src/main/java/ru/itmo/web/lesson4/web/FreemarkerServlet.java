...
public class FreemarkerServlet extends HttpServlet {
    ...
    private String getUri(HttpServletRequest request) throws UnsupportedEncodingException {
        return URLDecoder.decode(request.getRequestURI().equals("/")
                ? "/index" : request.getRequestURI(), UTF_8);
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            if (request.getRequestURI().equals("/")) {
                ...
                template = freemarkerConfiguration.getTemplate(getUri(request) + ".ftlh");
            }
        } catch (TemplateNotFoundException ignored) {
            template = freemarkerConfiguration.getTemplate("sc_not_found.ftlh");
        }
