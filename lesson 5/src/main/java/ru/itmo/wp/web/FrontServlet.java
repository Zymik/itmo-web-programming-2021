...
public class FrontServlet extends HttpServlet {
    ...
    private boolean checkMethodArgs(Method method) {
        return Arrays.stream(method.getParameterTypes())
                .allMatch(x -> x.isAssignableFrom(Map.class) || x.isAssignableFrom(HttpServletRequest.class));
    }
    ...
    private Method getMethod(Class<?> pageClass, String name) throws NotFoundException {
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            Optional<Method> m = Arrays.stream(clazz.getDeclaredMethods())
                    .filter(a -> a.getName().equals(name) && checkMethodArgs(a))
                    .findAny();
            if (m.isPresent()) {
                method = m.get();
            }
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            method.invoke(page, Arrays.stream(method.getParameterTypes())
                    .map(x -> x.isAssignableFrom(Map.class) ? view : request).toArray());
        ...
        renderTemplate(response, view, lang, pageClass.getSimpleName(),
                "Can't render template [pageClass=" + pageClass + ", action=" + method + "]");
    }

    private void renderTemplate(HttpServletResponse response, Map<String, Object> view,
                                String lang, String simpleTemplateName, String exceptionMessage)
            throws ServletException, IOException {
        ...
        if (!lang.equals("en")) {
            try {
                Template template = newTemplate(simpleTemplateName + "_" + lang + ".ftlh");
        ...
        if (!passed) {
            try {
                Template template = newTemplate(simpleTemplateName + ".ftlh");
                template.process(view, response.getWriter());
